...
st.markdown(
    ...
    /* ---- Wine result card: style Streamlit's native bordered container
       (st.container(border=True, key=...)) like a printed wine label.
       Streamlit turns a container's `key` into a CSS class "st-key-<key>",
       which is the officially supported way to target it -- this lets a
       native st.expander render visually inside the same box. ---- */
    div[class*="st-key-wine_card_"] {
        background: #FFFDF8;
        border: 1px solid #E4D3AE !important;
...
def render_recommendations(results):
    ...
    for index, wine in enumerate(results):
        with columns[index % 2]:
            ...
            winery = html.escape(str(wine.get("winery", "")).title())
            ...
            # st.container(border=True) groups the card content AND the
            # description expander below into a single visual box, so
            # "Show description" appears inside the wine card instead of
            # floating below it. The `key` gives this container a
            # predictable "st-key-wine_card_<id>" CSS class we can style.
            with st.container(border=True, key=f"wine_card_{wine_id}_{index}"):
                winery_line = f'<div class="wm-winery">{winery}</div>' if winery else ""
                st.markdown(
                    f"""
                    <div class="wm-card">
                        ...
                        <div class="wm-id">id: {wine_id}</div>
                    </div>
                    """,
                    unsafe_allow_html=True,
                )
